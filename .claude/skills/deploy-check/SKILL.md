...
3. Check: `/` and its `/assets/*.js` return 200; `/api/health` is `{"status":"ok"}`; `/api/portfolio` has 200 trades, no warnings, total P&L `-4401.3084972465`; the process user is `node`; `Content-Security-Policy` and `X-Content-Type-Options: nosniff` are present; the page renders in a browser with no console errors.
