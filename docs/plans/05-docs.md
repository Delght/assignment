...
- **Diagrams as Mermaid.**
  GitHub renders them from text, so they change in the same diff as the code: the architecture in the README, the import flow in the [API plan](02-api.md#import-flow).
- **My review is mine.**
...
- [x] Both diagrams render with Mermaid.
