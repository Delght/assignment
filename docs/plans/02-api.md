...
- **Decimal strings.**
  ...
  *Revised after review:* cutting at 20 places rounds twice, which can show $1.00 for an average just above $1.005; the API now sends full precision and only the browser rounds.
...
- The dataset became read-only by type, and keeps prices with their warnings as one snapshot instead of finding price warnings by their text.
